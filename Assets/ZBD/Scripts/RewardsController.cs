...
namespace ZBD
{
    public class RewardsController : MonoBehaviour
    {
            ...
            try
            {
                ...
                if (shouldVerify)
                {
                    StartPlayIntegrity(payloadHash, playIntegrityRes =>
                    {
                        ...
                        {
                            Debug.Log("Failed play integrity");
                        ...
                        {
                            Debug.Log("Not Failed play integrity");
        ...
        async void SendToGameServer(string token, string payload)
        {
            ...
            try
            {
                long  userid = long.Parse(ctx.Api.User.id + "");
                var result = await ctx.Microservices().GameServer().SendPlaytime(token, payload , userid);
